        ...
        def require_admin(f):
            ...
            def decorated_function(*args, **kwargs):
                ...
                try:
                    ...
                    logger.warning(
                        f"JWT validation failed: Invalid token. Error: {e}")
                    return jsonify({
                        "success": False,
                ...
                role = payload.get("role") or payload.get(
                    "user", {}).get("role")
                is_admin = payload.get("is_admin") or payload.get(
                    "user", {}).get("is_admin")
                ...
                logger.warning(
                    f"Access forbidden: User does not have admin privileges. Payload: {payload}")
                return jsonify({
                    "success": False,
