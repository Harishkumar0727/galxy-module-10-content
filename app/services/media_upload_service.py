...
def upload_to_folder(file, folder_name: str) -> str:
    ...
    try:
        ...
        logger.error(f"[{get_request_id()}] Upload attempt failed: File '{filename}' is corrupted or not a valid image. Error: {e}")
        raise CorruptedImageError(f"The uploaded file '{filename}' is corrupted or not a valid image.")
    ...
    if not detected_mime or detected_mime not in Config.ALLOWED_MIME_TYPES:
        logger.error(f"[{get_request_id()}] Upload attempt failed: Image format '{img_format}' (MIME: '{detected_mime}') is not allowed.")
        raise InvalidMimeTypeError(detected_mime or img_format, Config.ALLOWED_MIME_TYPES)

    # Validate dimensions (width/height > 0)
    width, height = img.size
    if width <= 0 or height <= 0:
        logger.error(f"[{get_request_id()}] Upload attempt failed: Image dimensions {width}x{height} are invalid.")
        raise InvalidImageDimensionsError("Image dimensions must be greater than zero.")

    logger.info(f"[{get_request_id()}] Image validation passed. Format: {img_format}, Dimensions: {width}x{height}.")
    file.seek(0)
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if attempt < max_retries and _should_retry(e):
                ...
                logger.warning(f"[{get_request_id()}] Cloudinary upload attempt {attempt + 1} failed: {e}. Retrying in {backoff_delay}s...")
                time.sleep(backoff_delay)
            else:
                logger.error(f"[{get_request_id()}] Cloudinary upload failed: {e}")
                break

    if not upload_details:
        if _is_timeout(last_exception):
            raise CloudinaryTimeoutError(f"Cloudinary upload service timed out after {max_retries + 1} attempts. Error: {last_exception}")
...
def delete_from_cloudinary(public_id: str) -> bool:
    ...
    try:
        ...
        if success:
            ...
            logger.warning(f"Delete response for '{public_id}' was not successful.")
            return False
    except Exception as e:
